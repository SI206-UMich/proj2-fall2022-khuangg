...
.
    ...
    for title in title_tag:
        place.append(title.text)

    cost_tag = soup.find_all('span', class_ = "_tyxjp1")
    for price in cost_tag:
        str_cost.append(price.text)

    for price in str_cost:
        money = price[1:]
        int_cost.append(int(money))


    id_listing = soup.find_all('div', class_ = "t1jojoys dir dir-ltr")
    for identification in id_listing:
        get_id = identification.get('id')[6:]
        id.append(get_id)
...
def check_policy_numbers(data):
    ...
    pattern1 = r'20[0-9]{2}-00[0-9]{4}STR'
    pattern2 = r'STR-000[0-9]{4}'
    ...
    for listing in data:
        ...
        found1 = re.findall(pattern1, id)
        found2 = re.findall(pattern2, id)

        if len(found1)>0 or len(found2)>0:
            correct_id.append(id)
        else:
            incorrect_id.append(listing[2])
        
    # print(incorrect_id)
    return list(incorrect_id)
...
class TestCases(unittest.TestCase):
    ...
    def test_check_policy_numbers(self):
        ...
        self.assertEqual(type(invalid_listings[0]) == str, True)
